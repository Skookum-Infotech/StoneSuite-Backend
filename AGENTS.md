# StoneSuite Backend — Codex Memory
...
`/feature <description>` (`.Codex/commands/feature.md`) drives a feature end-to-end:
...
hooks (`.Codex/hooks/`, zero tokens, deterministic only) → CI (zero tokens, needs real
execution) → haiku agents (`.Codex/agents/`, read-only, narrow judgement) → skills
(`.Codex/skills/`, domain knowledge, progressive disclosure via `references/`) →
superpowers → `/feature`.
