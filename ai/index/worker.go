...
type ChunkSink interface {
	Meta(ctx context.Context, sourceID string) (ai.ChunkMeta, bool, error)
	Upsert(ctx context.Context, c ai.Chunk) error
	UpdateScope(ctx context.Context, c ai.Chunk) error
...
func (w *Worker) process(ctx context.Context, j Job) error {
	...
	next := ai.Chunk{
		...
		Content: content, ContentHash: ai.VectorHash(w.fingerprint(), content),
	}
...
// detectable — see ai.VectorHash.
func (w *Worker) fingerprint() string {
	if f, ok := w.emb.(ai.Fingerprinter); ok {
...
func sameScope(prev ai.ChunkMeta, next ai.Chunk) bool {
...
// vector (ai.VectorHash), so a matching hash means the stored embedding is
...
func (w *Worker) reuseExisting(ctx context.Context, prev ai.ChunkMeta, next ai.Chunk) (done bool, err error) {
...
func (w *Worker) embedAndUpsert(ctx context.Context, c ai.Chunk) error {
