...
func (s *RagStore) Upsert(ctx context.Context, c rag.Chunk) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO rag_chunks (source_id, workflow_id, owner_user_id, team_id, record_type, content, content_hash, embedding, updated_at)
		VALUES ($1, NULLIF($2, '')::uuid, NULLIF($3, '')::uuid, NULLIF($4, '')::uuid, NULLIF($5, ''), $6, $7, $8, NOW())
		ON CONFLICT (source_id) DO UPDATE SET
			...
			updated_at    = NOW()`,
		c.SourceID, c.WorkflowID, c.OwnerUserID, c.TeamID, c.Kind, c.Content, c.ContentHash, pgvector.NewVector(c.Embedding))
	if err != nil {
...
func (s *RagStore) Meta(ctx context.Context, sourceID string) (m rag.ChunkMeta, found bool, err error) {
	...
	).Scan(&m.ContentHash, &m.WorkflowID, &m.OwnerUserID, &m.TeamID, &m.Kind)
	if err != nil {
...
// UpdateScope refreshes only the scope columns (and updated_at), leaving
// content and embedding untouched. This is the cheap path for a record whose
// text is unchanged but whose ownership or type moved — re-embedding identical
// text to correct an id would be pure waste, but leaving the stale value in
// place is a scope bug.
func (s *RagStore) UpdateScope(ctx context.Context, c rag.Chunk) error {
	_, err := s.pool.Exec(ctx, `
		UPDATE rag_chunks SET
			...
			record_type   = NULLIF($5, ''),
			updated_at    = NOW()
		WHERE source_id = $1`,
		c.SourceID, c.WorkflowID, c.OwnerUserID, c.TeamID, c.Kind)
	if err != nil {
