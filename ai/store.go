...
// ChunkMeta is the stored bookkeeping for one indexed record: what its text
// hashed to, and the scope columns retrieval ANDs the RBAC clause onto. It is
// deliberately everything-except-the-embedding, so the index worker can decide
// whether a re-embed is actually needed without paying to read a 768-wide
// vector back.
//
// Note the asymmetry that makes this subtle: ContentHash covers only what
// RenderRecord emits (workflow, state, core + custom fields). Owner and team
// are NOT rendered — they travel beside the text — so a record can change hands
// with its hash completely unchanged.
...
// compare directly against ai.Chunk's string fields, matching how Upsert writes
// them (NULLIF($n,'')::uuid).
func (s *RagStore) Meta(ctx context.Context, sourceID string) (m ChunkMeta, found bool, err error) {
	...
	if err != nil {
		...
		return ChunkMeta{}, false, fmt.Errorf("rag chunk meta: %w", err)
	}
	return m, true, nil
}
...
func (s *RagStore) UpdateScope(ctx context.Context, c Chunk) error {
	_, err := s.pool.Exec(ctx, `
