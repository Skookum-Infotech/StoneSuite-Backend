...
// Reject POST /api/tenant/vendor-bills/{uuid}/reject  body {"reason":"..."}
// Rejects the vendor bill named by {uuid}, which must be awaiting approval: it is sent back to Draft, keeping the
...
func (h *VendorBillOps) Reject(w http.ResponseWriter, r *http.Request) {
	uuid := r.PathValue("uuid")
	pool, identityID, _, ok := h.authVBByUUID(w, r, uuid, authz.ActionTransition)
	if !ok {
		return
	...
	isSuperAdmin, err := authz.IsSuperAdmin(r.Context(), pool, identityID)
	if err != nil {
		vbFail(w, err, "Failed to reject vendor bill.")
		return
	}
	empID := resolveEmployeeID(r, identityID)
	rec, err := vendorbill.Reject(r.Context(), pool, uuid, empID, isSuperAdmin, req.Reason)
	if err != nil {
		if errors.Is(err, vendorbill.ErrNotApprover) {
			logSecurityEvent(r, "approval_denied", "identity", identityID, "record", uuid)
		}
	...
	auditVB(r, pool, identityID, "reject", uuid, nil, rec)
	writeJSON(w, http.StatusOK, map[string]any{"success": true, "vendorBill": rec})
}
