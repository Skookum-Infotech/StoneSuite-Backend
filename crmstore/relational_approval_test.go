...
func TestCheckTransitionGate(t *testing.T) {
	tests := []struct {
		...
		{"gated, rejected, prospect lost exit -> allowed", 1, StatusRejected, "PCLL", false},
		{"gated, pending customer cannot be made active by hand", 1, StatusPending, "CACT", true},
		{"gated, pending customer cannot be made inactive", 1, StatusPending, "CINA", true},
		{"gated, rejected customer cannot go on credit hold", 1, StatusRejected, "CCHD", true},
		{"gated but approved -> allowed", 1, StatusApproved, "PDIS", false},
		{"gated, approved customer can be made inactive", 1, StatusApproved, "CINA", false},
		{"gated, approved, exit code too -> allowed", 1, StatusApproved, "PCLL", false},
