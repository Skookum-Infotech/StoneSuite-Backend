...
| **Area is not conserved on cut.** Saw kerf (3–5mm/cut) and dropped scrap are real losses | High | Parent's full area `consumed`; each retained offcut `recovered` at measured area; the shortfall recorded as `scrapped` with reason `SCRP`, so `SUM(delta)` still reconciles instead of drifting |
