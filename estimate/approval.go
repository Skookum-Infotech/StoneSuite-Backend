...
// approvalGateStatusCode is the checkpoint status an estimate passes through
// on its way to approvedStatusCode. Transition uses this to recognize a move
// onto the checkpoint with zero configured approvers and skip straight to
// approvedStatusCode instead, rather than parking the estimate on a status
...
// status (or a super admin override, see below), and is idempotent per
// (estimate, status, approver). Once every configured approver for the
// status has signed off (quorum -- all of them, not just one), the estimate
// auto-advances straight to the gate's Approved status in this same call;
// until then it stays in its current status, still gated, waiting on the
// rest.
//
...
func Approve(ctx context.Context, pool *pgxpool.Pool, uuid string, approverEmployeeID int, callerIsSuperAdmin bool) (*Estimate, error) {
	...
	// sign-off but leave the estimate in its current gated status.
	if _, err := tx.Exec(ctx, `
		UPDATE estimate SET estimate_approval_status = $2, estimate_updated_at = NOW()
		WHERE estimate_id = $1`, internalID, approvalPending); err != nil {
...
// finalizeApproval moves the estimate from curStatusID to the gate's fixed
...
func finalizeApproval(ctx context.Context, tx pgx.Tx, pool *pgxpool.Pool, uuid string, internalID, curStatusID, recordTypeID, approverEmployeeID int, historyAction string) (*Estimate, error) {
	...
	if _, err := tx.Exec(ctx, `
		UPDATE estimate SET
			estimate_status = $2, estimate_approval_status = $3, estimate_approved_by = $4,
			estimate_updated_at = NOW(), estimate_updated_by = $4, estimate_record_version = estimate_record_version + 1
		WHERE estimate_id = $1`, internalID, toStatusID, newApprovalStatus, nullableInt(approverEmployeeID)); err != nil {
		return nil, fmt.Errorf("finalize approve estimate: %w", err)
	}
...
// ApprovalInfo tells the detail page whether an estimate is actually gated
// right now, who is configured to sign off on it and who already has, and
// whether the requesting caller can approve it (AD-8) -- so the UI can show
// a banner instead of a transition control that would just 409.
type ApprovalInfo struct {
	...
	// moment an admin edits the approver list out from under an estimate
	...
	// CanReject is true while the estimate awaits approval and the caller is a
	// configured approver or a super admin -- one rejection is enough, no quorum.
	CanReject bool `json:"canReject"`
	// Rejection is the latest rejection, present while the estimate still sits in
...
// GetApprovalInfo resolves ApprovalInfo for an estimate. Returns Gated:
// false without error once the estimate isn't currently gated.
func GetApprovalInfo(ctx context.Context, pool *pgxpool.Pool, uuid string, callerEmployeeID int, callerIsSuperAdmin bool) (ApprovalInfo, error) {
	...
	err := pool.QueryRow(ctx, `
		SELECT estimate_id, estimate_status, estimate_approval_status FROM estimate
		WHERE estimate_uuid = $1 AND estimate_deleted_at IS NULL`, uuid).Scan(&internalID, &curStatusID, &approvalStatus)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	// Loaded before the gated checks: an estimate sent back to Draft is no longer
	...
	rows, err := pool.Query(ctx, `
		SELECT e.employee_id, COALESCE(NULLIF(u.full_name,''), u.email),
			EXISTS(SELECT 1 FROM estimate_approval eap
				WHERE eap.estimate_id = $3 AND eap.record_status_id = $2 AND eap.approver_employee_id = e.employee_id)
...
// Reject sends an estimate that is awaiting approval back to Draft on behalf of one of its
...
// controller to map.
func Reject(ctx context.Context, pool *pgxpool.Pool, uuid string, actorEmployeeID int, callerIsSuperAdmin bool, reason string) (*Estimate, error) {
