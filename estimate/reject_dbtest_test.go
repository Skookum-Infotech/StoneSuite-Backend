...
// pendingEstimate creates an estimate and submits it for approval.
func pendingEstimate(t *testing.T, pool *pgxpool.Pool) *Estimate {
	t.Helper()
	ctx := context.Background()
...
func storedRejections(t *testing.T, pool *pgxpool.Pool, uuid string) int {
	...
	if err := pool.QueryRow(context.Background(), `
		...
		JOIN estimate e ON e.estimate_id = ar.record_id
		JOIN lkp_record_type rt ON rt.record_type_id = ar.record_type_id AND rt.record_type_code = 'ESTM'
		WHERE e.estimate_uuid = $1`, uuid).Scan(&n); err != nil {
...
func TestReject_SendsBackToDraftAndTheReasonClearsOnResubmit(t *testing.T) {
	...
	est := pendingEstimate(t, pool)

	rejected, err := Reject(ctx, pool, est.ID, 1, false, "Pricing is off")
	if err != nil {
	...
	info, err := GetApprovalInfo(ctx, pool, est.ID, 1, false)
	if err != nil {
	...
	if info.Gated {
		t.Error("a Draft estimate is not gated")
	}

	// Submitting it again is a new round: the old rejection goes away for good
	// (an estimate cannot be recalled to Draft while its gate is open, so this
	// looks at the stored row rather than at a later recall).
	if _, err := Transition(ctx, pool, est.ID, "PAPV", 1); err != nil {
		t.Fatalf("resubmit: %v", err)
	}
	if n := storedRejections(t, pool, est.ID); n != 0 {
	...
	if info.Rejection != nil || !info.Gated || !info.CanApprove || !info.CanReject {
		t.Errorf("info = %+v, want the estimate gated and open to its approver again with no rejection", info)
	}
}
...
func TestReject_RequiresConfiguredApprover(t *testing.T) {
	...
	if _, err := Reject(ctx, pool, est.ID, 999999, false, "not mine to reject"); !errors.Is(err, ErrNotApprover) {
	...
	if _, err := Reject(ctx, pool, "00000000-0000-0000-0000-000000000000", 1, false, "x"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("Reject of a missing estimate = %v, want ErrNotFound", err)
	}
	if got, err := Get(ctx, pool, est.ID); err != nil || got.StatusCode != "PAPV" {
		t.Fatalf("after refused rejects: Get = %+v, %v; want the estimate still at PAPV", got, err)
	}
}
...
func TestApprove_SuperAdminOverrideIsRecordedInHistory(t *testing.T) {
	...
	approved, err := Approve(ctx, pool, est.ID, superAdmin, true)
	if err != nil {
	...
	if err := pool.QueryRow(ctx, `
		SELECT COUNT(*) FROM estimate_history h
		JOIN estimate e ON e.estimate_id = h.estimate_id
		WHERE e.estimate_uuid = $1 AND h.action = 'approve_override'`, est.ID).Scan(&n); err != nil {
...
// is recorded on the estimate itself, so it has to exist.
