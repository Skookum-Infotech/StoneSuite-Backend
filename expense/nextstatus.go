...
// fillNextStatusCodes sets NextStatusCodes on every claim given from one
...
func fillNextStatusCodes(ctx context.Context, q workflow.Querier, claims ...*Expense) error {
	if len(claims) == 0 {
		return nil
	}
	recordTypeID, err := recordTypeIDByCode(ctx, q, expenseRecordTypeCode)
	if err != nil {
		return fmt.Errorf("resolve EXPN record type: %w", err)
	}
