...
func Transition(ctx context.Context, pool *pgxpool.Pool, uuid, toStatusCode string, actorEmployeeID int) (*Job, error) {
	...
	// A move onto an approval checkpoint (Templating or QC Pending) with zero
	// approvers configured skips straight to the gate's approved target
	// instead of parking the job on a status literally labeled "Pending
	// Approval" that nothing is actually pending on -- mirrors
	// engine.finalize's own floor (StatusApproved) for landing on that
	// target.
	...
	notifyTransition(ctx, pool, uuid, st.internalID, recordTypeID, toStatusID, toStatusCode, requiredHere, st.approvalStatus, targetApprovers, actorEmployeeID)
	return Get(ctx, pool, uuid)
}

// notifyTransition best-effort-notifies approvers/owner around a manual
// status move: approvers when the job just entered a gated status, or the
// owner when a gated-unapproved job escaped via an always-allowed exit
// (e.g. Cancelled) instead of clearing approval.
...
// notifyCreated best-effort-notifies the actor who created a new
// fabrication job that the creation succeeded.
