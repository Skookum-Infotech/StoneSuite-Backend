...
//
// NOT for *_deleted_by columns: every one of them in this package is paired
// with its *_deleted_at by a strict CHECK, so NULL there is not a lost
// attribution but a constraint violation — a 500 on every soft delete, and the
// common case rather than an edge one, because resolveEmployeeID returns 0 for
// callers whose employee_user_id has never been populated. Use actorOrSystem
// (store.go) instead.
