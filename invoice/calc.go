...
// HeaderMoney holds an invoice's computed totals and balance.
type HeaderMoney struct {
	Subtotal, DiscountTotal, TaxTotal, GrandTotal, AmountPaid, BalanceDue float64
}

// ComputeHeader sums line money and applies shipping, adjustment, and computes balance due (spec §8).
func ComputeHeader(lines []LineMoney, shipping, adjustment, amountPaid float64) HeaderMoney {
	var h HeaderMoney
	for _, l := range lines {
