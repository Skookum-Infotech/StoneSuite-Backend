...
// seedInvoice inserts one invoice for a customer in the given status code.
func seedInvoice(t *testing.T, pool *pgxpool.Pool, customerID int, statusCode string, total float64) string {
	t.Helper()
	ctx := context.Background()
	// invoice_number is set explicitly: the real create path generates it in Go
	// after insert, and the column is nullable, but scanInvoice reads it into a
	// non-pointer string. Every real row has one.
	number := fmt.Sprintf("INV-%d", time.Now().UnixNano()%100000000)
	var uuid string
	err := pool.QueryRow(ctx, `
		INSERT INTO invoice (record_type, invoice_status, invoice_customer_id,
		                     invoice_created_by, invoice_grand_total, invoice_number)
		...
		WHERE rt.record_type_code = 'INVC' AND rs.record_status_code = $2
		RETURNING invoice_uuid`, customerID, statusCode, total, number).Scan(&uuid)
	if err != nil {
		t.Fatalf("seed %s invoice: %v", statusCode, err)
	}
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM invoice WHERE invoice_uuid = $1`, uuid)
	})
	return uuid
}
...
func TestPortalSearch_IsolatesCustomerAndHidesDrafts(t *testing.T) {
	...
	for _, code := range []string{"SENT", "PART", "PAID", "ODUE", "VOID"} {
		visible[code] = seedInvoice(t, pool, acme, code, 100)
	}
	hidden := map[string]string{}
	for _, code := range []string{"DRFT", "PAPV", "APPV"} {
		hidden[code] = seedInvoice(t, pool, acme, code, 100)
	}
	// Borealis: a SENT invoice that Acme must never see.
	borealisUUID := seedInvoice(t, pool, borealis, "SENT", 999)
	...
	for code, uuid := range visible {
		if !got[uuid] {
			t.Errorf("%s invoice missing from the portal view; it should be visible", code)
		}
	}
	for code, uuid := range hidden {
		if got[uuid] {
			t.Errorf("%s invoice is visible to the customer; unfinished documents must be hidden", code)
		}
	}
	if got[borealisUUID] {
		t.Error("SECURITY: another customer's invoice appeared in the portal view")
	}
...
func TestPortalSearch_HostileCustomerFilterCannotWiden(t *testing.T) {
	...
	acme := seedApprovedCustomer(t, pool, "Acme Stone")
	borealis := seedApprovedCustomer(t, pool, "Borealis Marble")
	seedInvoice(t, pool, acme, "SENT", 100)
	seedInvoice(t, pool, borealis, "SENT", 999)
...
func TestPortalGet_RefusesOtherCustomerAndHiddenStates(t *testing.T) {
	...
	ownSent := seedInvoice(t, pool, acme, "SENT", 100)
	ownDraft := seedInvoice(t, pool, acme, "DRFT", 100)
	othersSent := seedInvoice(t, pool, borealis, "SENT", 999)

	if _, err := PortalGet(ctx, pool, acme, ownSent); err != nil {
		t.Fatalf("PortalGet on own visible invoice: %v", err)
	}

	for name, uuid := range map[string]string{
		"own draft":                  ownDraft,
		"another customer's invoice": othersSent,
	} {
...
func TestPortalGet_ReturnsFullBodyForRendering(t *testing.T) {
	...
	acme := seedApprovedCustomer(t, pool, "Acme Stone")
	uuid := seedInvoice(t, pool, acme, "SENT", 250)
