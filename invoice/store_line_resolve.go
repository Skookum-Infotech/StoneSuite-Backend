...
func resolveLine(ctx context.Context, pool *pgxpool.Pool, in InvoiceLineInput, headerTaxPercent float64) (resolvedLine, error) {
	if in.LineNumber <= 0 {
	...
	// both the item name and the description.
	rl := resolvedLine{in: in, itemName: in.Description}
	...
	rl.money = ComputeLine(LineInput{Quantity: in.Quantity, UnitPrice: in.UnitPrice, DiscountPercent: in.DiscountPercent, TaxPercent: taxPercent})
	return rl, nil
}
