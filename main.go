...
func main() {
	...
	globalHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		path := r.URL.Path
		if path != "/api" && !strings.HasPrefix(path, "/api/auth/") && !strings.HasPrefix(path, "/api/customers") && !strings.HasPrefix(path, "/api/onboarding") && !strings.HasPrefix(path, "/api/leads") && !strings.HasPrefix(path, "/api/tenant") && !strings.HasPrefix(path, "/api/platform") {
			w.Header().Set("Content-Type", "application/json")
