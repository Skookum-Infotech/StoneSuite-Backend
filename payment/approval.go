...
var ErrNotApprover = errors.New("you are not a configured approver for this payment's current status")

// ErrApprovalRequired maps to HTTP 409.
var ErrApprovalRequired = errors.New("this payment must be approved before it can leave its current status")

// ErrApprovalNotRequired maps to HTTP 409.
var ErrApprovalNotRequired = errors.New("this payment's current status does not require approval")

// moduleConfig resolves the shared approvalchain.ModuleConfig for Payment
// (workflows.key "payment") once, so callers don't repeat the
// ForWorkflowKey lookup+panic-guard.
func moduleConfig() approvalchain.ModuleConfig {
	cfg, ok := approvalchain.ForWorkflowKey("payment")
	if !ok {
		panic("approvalchain: \"payment\" is not registered")
	}
	return cfg
}

// Approve records one approver's sign-off on a payment at its current gate
// (PEND, AD-8) via the shared approvalchain engine. Once every configured
// approver has signed off -- or a super admin overrides -- the payment
// auto-advances to the gate's target status in the same call.
func Approve(ctx context.Context, pool *pgxpool.Pool, uuid string, approverEmployeeID int, callerIsSuperAdmin bool) (*Payment, error) {
...
// GetApprovalInfo resolves approvalchain.ApprovalInfo for a payment -- who
...
// Reject vetoes a payment that is awaiting approval on behalf of one of
// its configured approvers (or a super admin) via the shared approvalchain
// engine, keeping who rejected it and why. A payment's approval gate sits on its
...
// controller to map.
func Reject(ctx context.Context, pool *pgxpool.Pool, uuid string, actorEmployeeID int, callerIsSuperAdmin bool, reason string) (*Payment, error) {
