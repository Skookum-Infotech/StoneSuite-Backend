...
func portalWhere(customerID int) ([]string, []any, int, error) {
	...
	if !ok {
		...
		return nil, nil, 0, errors.New("portal visibility not configured for payment")
	}
	where := []string{
		"p.payment_deleted_at IS NULL",
		"p.payment_customer_id = $1",
...
// PortalGet share headerSelect/scanPayment with the staff-facing Search/Get —
...
func redactForPortal(rec *Payment) *Payment {
	rec.InternalNotes = ""
...
func PortalSearch(ctx context.Context, pool *pgxpool.Pool, customerID int, req query.Request) (Page, error) {
	...
	if err != nil {
		return Page{}, fmt.Errorf("portal search payment: %w", err)
	}
	defer rows.Close()
...
func PortalGet(ctx context.Context, pool *pgxpool.Pool, customerID int, uuid string) (*Payment, error) {
	where, args, nextIdx, err := portalWhere(customerID)
	if err != nil {
		return nil, err
	}
	where = append(where, fmt.Sprintf("p.payment_uuid = $%d", nextIdx))
	args = append(args, uuid)

	q := headerSelect + " WHERE " + strings.Join(where, " AND ")

	rec, meta, err := scanPayment(pool.QueryRow(ctx, q, args...))
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("portal get payment: %w", err)
	}
