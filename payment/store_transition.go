...
func Transition(ctx context.Context, pool *pgxpool.Pool, id, toStatusCode string, actorEmployeeID int) (*Payment, error) {
	...
	// payment on a status literally labeled "Pending Approval" that nothing
	...
	// Any move takes the payment out of the state a Reject left it in.
...
// notifyCreated best-effort-notifies the actor who created a new payment
