...
// List returns prospects for the tenant, newest first, filtered by the caller's
// RBAC scope:
//   - "all":  every prospect
//   - "team": prospects the caller owns OR assigned to one of the caller's teams
//   - "own":  prospects the caller owns
func List(ctx context.Context, q Querier, scope, callerUserID string, teamIDs []string) ([]Prospect, error) {
	var (
		rows pgx.Rows
		err  error
		base = `SELECT ` + selectCols + ` FROM prospects`
	)
	switch scope {
	case "all":
