...
func Transition(ctx context.Context, pool *pgxpool.Pool, uuid, toStatusCode string, actorEmployeeID int) (*PurchaseOrder, error) {
	...
	// order on a status literally labeled "Pending Approval" that nothing
	...
	// Any move takes the order out of the state a Reject left it in.
...
// notifyCreated best-effort-notifies the actor who created a new purchase
// order that the creation succeeded.
