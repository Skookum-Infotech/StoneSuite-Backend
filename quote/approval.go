...
type ApprovalInfo struct {
	...
	// CanReject is true while the quote awaits approval and the caller is a
	// configured approver or a super admin -- one rejection is enough, no quorum.
	CanReject bool `json:"canReject"`
	// Rejection is the latest rejection, present while the quote still sits in
...
func GetApprovalInfo(ctx context.Context, pool *pgxpool.Pool, uuid string, callerEmployeeID int, callerIsSuperAdmin bool) (ApprovalInfo, error) {
	...
	// Loaded before the gated checks: a quote sent back to Draft is no longer
...
// Reject sends a quote that is awaiting approval back to Draft on behalf of one of its
...
// controller to map.
func Reject(ctx context.Context, pool *pgxpool.Pool, uuid string, actorEmployeeID int, callerIsSuperAdmin bool, reason string) (*Quote, error) {
