...
// serial PK, zero-padded to 6 digits (spec AD-11): QUOT-000001. Used as the
// fallback when the "quote" workflow has no Record Numbering config enabled.
func FormatNumber(serialID int64) string {
	return fmt.Sprintf("%s-%06d", numberPrefix, serialID)
}

// numberTarget wires quotes into the per-workflow Record Numbering config
// (Configure → Record Numbering, workflow_numbering_configs).
var numberTarget = workflow.NumberTarget{
	WorkflowKey: "quote",
	UpdateSQL:   `UPDATE quote SET quote_number = $1 WHERE quote_id = $2`,
	Fallback:    FormatNumber,
}

// assignNumber claims a quote's record number — the configured format when
// numbering is enabled for the "quote" workflow, otherwise FormatNumber — and
// writes it to the row. Must run inside the create/convert transaction. A
// misconfigured number (collides with an existing quote, or too long for the
