...
// ErrNotFound is returned when an quote uuid matches nothing live.
...
// the internal id on hand from the quote row).
...
// zip). prefix is "quote_bill" or "quote_ship".
...
const itemSelect = `
	SELECT ei.quote_item_uuid, ei.line_number,
	...
	WHERE ei.quote_id = $1 AND ei.item_deleted_at IS NULL
...
// loadLines fetches an quote's live lines by its external uuid.
