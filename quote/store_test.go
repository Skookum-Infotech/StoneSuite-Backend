...
func TestTransition_ToCheckpointWithNoApprovers_SkipsStraightToApproved(t *testing.T) {
	...
	// quote_approver is config-level (keyed by record_type_id/status_id, not
	// per-quote) and other tests in this package seed rows there without
	...
	if _, err := pool.Exec(ctx, `DELETE FROM quote_approver WHERE record_type_id = $1 AND record_status_id = $2`, recordTypeID, papvStatusID); err != nil {
		t.Fatalf("clear quote_approver: %v", err)
	}
	...
	// No quote_approver rows configured for (QUOT, PAPV), so submitting for
	// approval should land straight on APPV rather than parking the quote on
	// an unresolvable "Pending Approval".
	updated, err := Transition(ctx, pool, created.ID, "PAPV", 1)
	if err != nil {
	...
	// The quote is no longer gated (it's not even at PAPV anymore), so
