...
// seedRefund inserts one invoice for a customer in the given status code.
func seedRefund(t *testing.T, pool *pgxpool.Pool, customerID int, statusCode string, total float64) string {
	t.Helper()
	ctx := context.Background()
	// refund_number is set explicitly: the real create path generates it in Go
	// after insert, and the column is nullable, but scanInvoice reads it into a
	// non-pointer string. Every real row has one.
	number := fmt.Sprintf("RFN-%d", time.Now().UnixNano()%100000000)
	var uuid string
	err := pool.QueryRow(ctx, `
		INSERT INTO refund (record_type, refund_status, refund_customer_id,
		                     refund_created_by, refund_amount, refund_number, refund_method)
		...
		WHERE rt.record_type_code = 'RFND' AND rs.record_status_code = $2
		RETURNING refund_uuid`, customerID, statusCode, total, number).Scan(&uuid)
	if err != nil {
		t.Fatalf("seed %s refund: %v", statusCode, err)
	}
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM refund WHERE refund_uuid = $1`, uuid)
	})
	return uuid
}
...
func TestPortalSearch_IsolatesCustomerAndHidesDrafts(t *testing.T) {
	...
	for _, code := range []string{"APPV", "SENT", "VOID"} {
		visible[code] = seedRefund(t, pool, acme, code, 100)
	}
	hidden := map[string]string{}
	for _, code := range []string{"PEND"} {
		hidden[code] = seedRefund(t, pool, acme, code, 100)
	}
	// Borealis: a SENT invoice that Acme must never see.
	borealisUUID := seedRefund(t, pool, borealis, "APPV", 999)
	...
	for code, uuid := range visible {
		if !got[uuid] {
			t.Errorf("%s refund missing from the portal view; it should be visible", code)
		}
	}
	for code, uuid := range hidden {
		if got[uuid] {
			t.Errorf("%s refund is visible to the customer; unfinished documents must be hidden", code)
		}
	}
	if got[borealisUUID] {
		t.Error("SECURITY: another customer's refund appeared in the portal view")
	}
...
func TestPortalSearch_HostileCustomerFilterCannotWiden(t *testing.T) {
	...
	acme := seedApprovedCustomer(t, pool, "Acme Stone")
	borealis := seedApprovedCustomer(t, pool, "Borealis Marble")
	seedRefund(t, pool, acme, "APPV", 100)
	seedRefund(t, pool, borealis, "APPV", 999)
...
func TestPortalGet_RefusesOtherCustomerAndHiddenStates(t *testing.T) {
	...
	ownSent := seedRefund(t, pool, acme, "APPV", 100)
	ownDraft := seedRefund(t, pool, acme, "PEND", 100)
	othersSent := seedRefund(t, pool, borealis, "APPV", 999)

	if _, err := PortalGet(ctx, pool, acme, ownSent); err != nil {
		t.Fatalf("PortalGet on own visible refund: %v", err)
	}

	for name, uuid := range map[string]string{
		"own hidden-state refund":   ownDraft,
		"another customer's refund": othersSent,
	} {
...
func TestPortalGet_ReturnsFullBodyForRendering(t *testing.T) {
	...
	acme := seedApprovedCustomer(t, pool, "Acme Stone")
	uuid := seedRefund(t, pool, acme, "APPV", 250)
