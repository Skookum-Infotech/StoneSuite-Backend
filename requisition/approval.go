// requisition/approval.go — AD-7: the configuration-driven approval gate,
...
// Requisition (workflows.key "requisition") once, so callers don't repeat
// the ForWorkflowKey lookup+panic-guard.
func moduleConfig() approvalchain.ModuleConfig {
	cfg, ok := approvalchain.ForWorkflowKey("requisition")
	if !ok {
		panic("approvalchain: \"requisition\" is not registered")
	}
	return cfg
}

// Approve records one approver's sign-off on a requisition at its current
// gate (PAPV, AD-7) via the shared approvalchain engine. Once every
// configured approver has signed off -- or a super admin overrides -- the
// requisition auto-advances to the gate's target status in the same call.
func Approve(ctx context.Context, pool *pgxpool.Pool, uuid string, approverEmployeeID int, callerIsSuperAdmin bool) (*Requisition, error) {
...
// GetApprovalInfo resolves approvalchain.ApprovalInfo for a requisition --
