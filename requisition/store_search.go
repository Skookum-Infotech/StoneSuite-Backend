...
// Search lists requisitions under the caller's RBAC scope with filter/sort/
// global search + keyset pagination. Scope × filter is ANDed — a filter can
// only narrow the permitted set. "own" scope narrows to requisitions the
// caller requested (AD-5). List rows omit line items to avoid an N+1 join.
func Search(ctx context.Context, pool *pgxpool.Pool, scope, actorIdentityID string, req query.Request) (Page, error) {
	where := []string{"reqn.requisition_deleted_at IS NULL"}
	var args []any
	nextIdx := 1
	if scope != string(authz.ScopeAll) {
		...
		where = append(where, fmt.Sprintf("reqn.requisition_requested_by_id = $%d", nextIdx))
	...
	q := reqnSelect + " WHERE " + strings.Join(where, " AND ") +
		" ORDER BY " + built.OrderBy + " LIMIT " + strconv.Itoa(built.EffLimit+1)

	rows, err := pool.Query(ctx, q, args...)
	if err != nil {
		return Page{}, fmt.Errorf("search requisitions: %w", err)
	}
	defer rows.Close()
	out := []Requisition{}
	metas := []reqnMeta{}
	for rows.Next() {
		p, meta, err := scanRequisition(rows)
		if err != nil {
			return Page{}, err
		}
		out = append(out, *p)
...
// sortValue reads the effective sort field's value from a requisition to
...
// does not carry, which is what reqnMeta is for.
// TestSortValueCoversEverySortableField guards the correspondence.
func sortValue(p Requisition, meta reqnMeta, field string) any {
	switch field {
	case "estimated_total":
		return p.EstimatedTotal
