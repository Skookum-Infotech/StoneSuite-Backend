...
func configureApprover(t *testing.T, pool *pgxpool.Pool) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(),
			`DELETE FROM sales_order_approver WHERE record_type_id = $1 AND record_status_id = $2`, recordTypeID, papvStatusID)
	})
}

// pendingOrder creates a sales order and submits it for approval.
func pendingOrder(t *testing.T, pool *pgxpool.Pool) *Order {
	t.Helper()
	ctx := context.Background()
...
func storedRejections(t *testing.T, pool *pgxpool.Pool, uuid string) int {
	...
	if err := pool.QueryRow(context.Background(), `
		...
		JOIN sales_order so ON so.sales_order_id = ar.record_id
		JOIN lkp_record_type rt ON rt.record_type_id = ar.record_type_id AND rt.record_type_code = 'SORD'
		WHERE so.sales_order_uuid = $1`, uuid).Scan(&n); err != nil {
...
func TestReject_SendsBackToDraftAndTheReasonClearsOnResubmit(t *testing.T) {
	...
	so := pendingOrder(t, pool)

	rejected, err := Reject(ctx, pool, so.ID, 1, false, "Credit limit exceeded")
	if err != nil {
	...
	info, err := GetApprovalInfo(ctx, pool, so.ID, 1, false)
	if err != nil {
		t.Fatalf("GetApprovalInfo: %v", err)
	}
	if info.Rejection == nil || info.Rejection.Reason != "Credit limit exceeded" {
	...
	// (an order cannot be recalled to Draft while its gate is open, so this
	// looks at the stored row rather than at a later recall).
	if _, err := Transition(ctx, pool, so.ID, "PAPV", 1); err != nil {
		t.Fatalf("resubmit: %v", err)
	}
	if n := storedRejections(t, pool, so.ID); n != 0 {
	...
	if info.Rejection != nil || !info.Gated || !info.CanApprove || !info.CanReject {
		t.Errorf("info = %+v, want the order gated and open to its approver again with no rejection", info)
	}
}
...
func TestReject_RequiresConfiguredApprover(t *testing.T) {
	...
	if _, err := Reject(ctx, pool, so.ID, 999999, false, "not mine to reject"); !errors.Is(err, ErrNotApprover) {
	...
	if got, err := Get(ctx, pool, so.ID); err != nil || got.StatusCode != "PAPV" {
...
func TestApprove_SuperAdminOverrideIsRecordedInHistory(t *testing.T) {
	...
	approved, err := Approve(ctx, pool, so.ID, superAdmin, true)
	if err != nil {
	...
	if err := pool.QueryRow(ctx, `
		SELECT COUNT(*) FROM sales_order_history h
		JOIN sales_order s ON s.sales_order_id = h.sales_order_id
		WHERE s.sales_order_uuid = $1 AND h.action = 'approve_override'`, so.ID).Scan(&n); err != nil {
...
// is recorded on the order itself, so it has to exist.
