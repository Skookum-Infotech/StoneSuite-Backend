...
// mirroring quote/store_test.go's helper of the same name.
...
func TestCreate_SnapshotsAndTotals(t *testing.T) {
	...
	if got.Number == "" || got.Number[:5] != "SORD-" {
		t.Errorf("Number = %q, want SORD- prefix", got.Number)
	}
...
func TestCreate_RequiresCustomer(t *testing.T) {
	pool := testPool(t)
	_, err := Create(context.Background(), pool, CreateOrderInput{}, 1)
...
func TestUpdate_RecomputesTotalsAndBumpsVersion(t *testing.T) {
	...
	updated, err := Update(context.Background(), pool, created.ID, UpdateOrderInput{
		orderFields: orderFields{
			SalesTaxPercent: 0,
			Items:           []LineInput2{{LineNumber: 1, InventoryItemUUID: itemUUID, Quantity: 3}},
		},
	}, 1)
	if err != nil {
...
func TestApprove_SignOffFlipsApprovalStatus(t *testing.T) {
	...
	// met (salesorder/approval.go's finalizeApproval), so ApprovalStatus now
