...
// Embedder embeds text via a self-hosted Ollama instance (POST
...
// NewDocEmbedder builds an embedder for STORED text. Use it in the
// ingestion worker. dim is the expected vector width (config.AIEmbedDim); pass
// 0 to skip validation.
func NewDocEmbedder(baseURL, model string, dim int) *Embedder {
	return newEmbedder(baseURL, model, prefixesFor(model).doc, dim)
}

// NewQueryEmbedder builds an embedder for QUESTIONS. Use it in the
// retriever. dim is the expected vector width; pass 0 to skip validation.
func NewQueryEmbedder(baseURL, model string, dim int) *Embedder {
	return newEmbedder(baseURL, model, prefixesFor(model).query, dim)
}

func newEmbedder(baseURL, model, prefix string, dim int) *Embedder {
	return &Embedder{
		baseURL:    baseURL,
		model:      model,
		prefix:     prefix,
...
func (e *Embedder) Fingerprint() string { return e.model + "\x00" + e.prefix }
