...
func configureApprover(t *testing.T, pool *pgxpool.Pool) {
	...
	if err := pool.QueryRow(ctx, `SELECT record_type_id FROM lkp_record_type WHERE record_type_code = 'VBIL'`).Scan(&recordTypeID); err != nil {
		t.Fatalf("resolve VBIL record type: %v", err)
	}
	...
	if _, err := pool.Exec(ctx, `
		INSERT INTO vendor_bill_approver (record_type_id, record_status_id, approver_employee_id)
		VALUES ($1, $2, 1) ON CONFLICT DO NOTHING`, recordTypeID, papvStatusID); err != nil {
		t.Fatalf("seed vendor_bill_approver: %v", err)
	}
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(),
			`DELETE FROM vendor_bill_approver WHERE record_type_id = $1 AND record_status_id = $2`, recordTypeID, papvStatusID)
	})
}

// pendingBill creates a vendor bill and submits it for approval.
func pendingBill(t *testing.T, pool *pgxpool.Pool) *VendorBill {
	t.Helper()
	ctx := context.Background()
	...
	pending, err := Transition(ctx, pool, bill.ID, "PAPV", 1)
	if err != nil {
...
func TestReject_SendsBackToDraftAndTheReasonClearsOnResubmit(t *testing.T) {
	...
	bill := pendingBill(t, pool)

	rejected, err := Reject(ctx, pool, bill.ID, 1, false, "Duplicate of VND-INV-001")
	if err != nil {
	...
	info, err := GetApprovalInfo(ctx, pool, bill.ID, 1, false)
	if err != nil {
		t.Fatalf("GetApprovalInfo: %v", err)
	}
	if info.Rejection == nil || info.Rejection.Reason != "Duplicate of VND-INV-001" {
	...
	if !info.Gated || !info.CanApprove || !info.CanReject {
		t.Errorf("info = %+v, want the bill gated and open to its approver again", info)
	}

	// Recalling it to Draft is not a rejection: the old reason must not resurface.
	if _, err := Transition(ctx, pool, bill.ID, "DRFT", 1); err != nil {
...
func TestReject_RequiresConfiguredApprover(t *testing.T) {
	...
	if _, err := Reject(ctx, pool, bill.ID, 999999, false, "not mine to reject"); !errors.Is(err, ErrNotApprover) {
	...
	if _, err := Reject(ctx, pool, "00000000-0000-0000-0000-000000000000", 1, false, "x"); !errors.Is(err, ErrNotFound) {
		t.Fatalf("Reject of a missing bill = %v, want ErrNotFound", err)
	}
	if got, err := Get(ctx, pool, bill.ID); err != nil || got.StatusCode != "PAPV" {
		t.Fatalf("after refused rejects: Get = %+v, %v; want the bill still at PAPV", got, err)
	}
}
