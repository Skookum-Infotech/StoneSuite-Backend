...
// Reject vetoes a vendor credit that is awaiting approval on behalf of one of
// its configured approvers (or a super admin) via the shared approvalchain
// engine, keeping who rejected it and why. A vendor credit's approval gate sits on its
...
// controller to map.
func Reject(ctx context.Context, pool *pgxpool.Pool, uuid string, actorEmployeeID int, callerIsSuperAdmin bool, reason string) (*VendorCredit, error) {
