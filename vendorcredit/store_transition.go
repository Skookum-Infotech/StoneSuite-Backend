...
func Transition(ctx context.Context, pool *pgxpool.Pool, id, toStatusCode string, actorEmployeeID int) (*VendorCredit, error) {
	...
	notifyTransition(ctx, pool, id, internalID, recordTypeID, toStatusID, toStatusCode, requiredHere, approvalStatus, targetApprovers, actorEmployeeID)
	return Get(ctx, pool, id)
}

// notifyTransition best-effort-notifies approvers/owner around a manual
// status move (AD-8): approvers when the vendor credit just entered a
// gated status, or the owner when a gated-unapproved credit escaped via an
...
// credit that the creation succeeded.
