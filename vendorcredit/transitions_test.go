...
func TestCanTransition(t *testing.T) {
	tests := []struct {
		...
		// APPL->VOID is absent by design: an exhausted credit must be
		// reversed first (which returns it to APPV), so the void cascade
		// stays bounded and "this credit was spent" stays terminal.
...
// The lifecycle is fixed by the lkp_record_status seed for record_type 17,
// the exact same status set as CRDT/credit_memo. This pins the map to those
// four codes so a stray edit is caught here.
