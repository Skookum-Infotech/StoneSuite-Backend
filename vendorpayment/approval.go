...
// Payment (workflows.key "vendor_payment") once, so callers don't repeat
// the ForWorkflowKey lookup+panic-guard.
func moduleConfig() approvalchain.ModuleConfig {
	cfg, ok := approvalchain.ForWorkflowKey("vendor_payment")
	if !ok {
		panic("approvalchain: \"vendor_payment\" is not registered")
	}
	return cfg
}

// Approve records one approver's sign-off on a vendor payment at its
...
// payment auto-advances to the gate's target status (APPV) in the same
// call, same as every other module built on this engine.
func Approve(ctx context.Context, pool *pgxpool.Pool, uuid string, approverEmployeeID int, callerIsSuperAdmin bool) (*VendorPayment, error) {
...
// GetApprovalInfo resolves approvalchain.ApprovalInfo for a vendor payment
...
// Reject sends a vendor payment that is awaiting approval back to Draft on
...
func Reject(ctx context.Context, pool *pgxpool.Pool, uuid string, actorEmployeeID int, callerIsSuperAdmin bool, reason string) (*VendorPayment, error) {
